"""Fetcher for finding OpenAI Codex PRs on GitHub."""
...
class CodexPRFetcher:
    """Fetcher for finding OpenAI Codex PRs across GitHub.

    OpenAI Codex creates branches named `codex/...` when making PRs.
    GitHub's search API doesn't support prefix matching (`head:codex/`),
    so we fetch all PRs with `head:codex` and filter locally.

    The strategy:
    1. Use REST API issue search with `is:pr head:codex`
    2. Date-split to overcome 1000 result limit
    3. Filter locally for branches starting with `codex/`
    """

    # Base search query for Codex PRs
    ...
    def __init__(self, token: str):
        """
        Initialize the Codex PR fetcher.
    ...
    def fetch_codex_prs(
        ...
        """
        Fetch all PRs with 'codex' in branch name.

        Uses date-splitting to overcome 1000 result limit.
        Optionally filters locally for branches starting with 'codex/'.

        Args:
            start_date: Start of date range (default: May 2025, Codex launch)
            end_date: End of date range (default: today)
            output_path: Optional path for incremental saves
            filter_prefix: If True, filter for 'codex/' prefix (default: True)
        ...
        print(f"Searching for Codex PRs...")
        print(f"Date range: {start_date} to {end_date}")
        print(f"Query: {self.SEARCH_QUERY}")
        print(f"Filter for 'codex/' prefix: {filter_prefix}")
        print()
        ...
        try:
            ...
            print(f"Estimated total PRs (with 'codex' in branch): {total_estimate:,}")
            if total_estimate == 0:
                print("No PRs found.")
                return []
    ...
    def _fetch_prs_with_date_split(
        ...
        if filter_prefix:
            print(f"\n  Filtered out {filtered_count:,} PRs without 'codex/' prefix")
    ...
    def _is_codex_pr(self, pr_data: Dict[str, Any]) -> bool:
        """
        Filter for branches starting with 'codex/'.

        Args:
            pr_data: PR data dictionary

        Returns:
            True if head_ref starts with 'codex/'
        """
        head_ref = pr_data.get('head_ref', '')
        return head_ref.startswith('codex/')
    ...
    def fetch_and_enrich_prs(
        ...
        """
        Fetch PRs and enrich each with branch information.

        This is slower as it requires an extra API call per PR,
        but provides accurate branch filtering.

        Args:
            start_date: Start of date range
            end_date: End of date range (default: today)
            output_path: Optional path for saves

        Returns:
            List of enriched PR data dictionaries
        """
        if end_date is None:
            end_date = date.today()

        # First fetch all PRs without filtering (we'll filter after enrichment)
        print("Phase 1: Fetching all PRs with 'codex' in branch name...")
        all_prs = self.fetch_codex_prs(
            start_date=start_date,
            end_date=end_date,
            output_path=None,  # Don't save yet
            filter_prefix=False  # Can't filter without branch info
        )

        if not all_prs:
            return []

        # Phase 2: Enrich each PR with branch info
        print(f"\nPhase 2: Enriching {len(all_prs):,} PRs with branch information...")
        enriched_prs = []

        for pr in tqdm(all_prs, desc="Enriching PRs", unit="pr"):
            enriched = self._enrich_pr_with_branch(pr)
            if self._is_codex_pr(enriched):
                enriched_prs.append(enriched)

        self._prs = enriched_prs

        print(f"\nFiltered to {len(enriched_prs):,} PRs with 'codex/' prefix")
    ...
    def get_unique_repos(self) -> pd.DataFrame:
        """
        Get unique repos with Codex PR counts.

        Returns:
            DataFrame with repo_nwo, codex_pr_count, first_codex_pr, last_codex_pr
        """
        if not self._prs:
            return pd.DataFrame()
        ...
        return repo_stats.sort_values('codex_pr_count', ascending=False)

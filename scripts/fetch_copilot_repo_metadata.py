#!/usr/bin/env python3
"""Parallel fetcher for Copilot repo metadata using batched GraphQL alias queries.

Reads the unique repo list from copilot_repos.csv and fetches
full metadata (33 columns + README) for each repo using parallel workers.

Usage:
    python scripts/fetch_copilot_repo_metadata.py
    python scripts/fetch_copilot_repo_metadata.py --no-readme   # skip README for speed
    python scripts/fetch_copilot_repo_metadata.py --dry-run      # show plan only
"""
...
def load_copilot_repos(repos_file: Path) -> List[str]:
    """Load unique repo names from Copilot repos CSV file."""
    print(f"Loading Copilot repos from {repos_file}...")
    df = pd.read_csv(repos_file)
    repos = df["repo_nwo"].tolist()
...
def merge_results(temp_dir: Path, output_file: Path) -> int:
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        for worker_file in sorted(temp_dir.glob("copilot_repos_worker_*.jsonl")):
            count = 0
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch repo metadata for all Copilot repos"
    )
    parser.add_argument(
        "--repos-file",
        type=str,
        default="data/output/copilot_repos.csv",
        help="Input repos CSV file",
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
        default="data/output/copilot_repos_metadata.jsonl",
    ...
    repo_list = load_copilot_repos(repos_file)
    ...
    temp_dir = output_path.parent / "temp_copilot_repo_workers"

    print("\n" + "=" * 70)
    print("COPILOT REPO METADATA FETCH PLAN")
    print("=" * 70)
    ...
    worker_args = []
    for i, chunk in enumerate(chunks):
        temp_file = temp_dir / f"copilot_repos_worker_{i+1}.jsonl"
        worker_args.append((
            i + 1,
            tokens[i][1],
            chunk,

"""
Visualize Spotify collaboration network with Claude Code adoption over time.
...
print("=" * 70)
print("SPOTIFY NETWORK VISUALIZATION")
print("=" * 70)

# Load data
print("\n1. Loading data...")
contributors_df = pd.read_parquet("data/output/spotify_contributors.parquet")
all_users_df = pd.read_parquet("data/output/spotify_claude_users.parquet")
...
claude_users_df['first_claude_use'] = pd.to_datetime(claude_users_df['first_claude_use'])
...
# Filter to Claude users in LCC for visualization (full network too large)
...
# For visualization, we'll create a subgraph around Claude users
# Get all Claude users + their direct neighbors (1-hop)
...
# Limit neighbors to keep visualization manageable
# Keep neighbors with highest degree
...
top_neighbors = set([n for n, d in neighbor_degrees[:200]])  # Top 200 neighbors
...
<head>
    <meta charset="utf-8">
    <title>Spotify Network - Claude Code Adoption</title>
...
<body>
    <h1>🎵 Spotify Network: Claude Code Adoption</h1>
    <p class="subtitle">Tracking how Claude Code spread through Spotify's collaboration network</p>
    ...
    <div id="container">
        ...
        <div id="controls">
            ...
            <div id="stats">
                <div>Claude Users: <span class="stat-value" id="adopted-count">0</span></div>
                <div>Total Nodes: <span class="stat-value" id="total-nodes">0</span></div>
                <div>Connections: <span class="stat-value" id="total-edges">0</span></div>
            </div>

            <div class="legend">
                <div class="legend-item">
                    <div class="legend-dot" style="background: #1DB954;"></div>
        ...
        nodeElements
            ...
            .attr("fill", d => {
                if (adoptedUsers.has(d.id)) {
                    ...
                    return "#1DB954";  // Existing Claude user
                }
                return "#555";  // Non-user
            })
            .attr("r", d => {
                if (adoptedUsers.has(d.id) && !prevAdoptedUsers.has(d.id)) {
                    return 12;  // Bigger for new adopters
                }
...
for node in G_viz.nodes():
    ...
    nodes_data.append({
        ...
        'y': float(y),
        'isClaude': is_claude,
        'adoptionDate': adoption_date.strftime('%Y-%m-%d') if adoption_date else None
    })
...
adoption_dates_str = {k: v.strftime('%Y-%m-%d') for k, v in adoption_dates.items() if k in viz_nodes}
...
# Save HTML
html_path = output_dir / "spotify_network_animation.html"
...
# Save GEXF
gexf_path = output_dir / "spotify_network.gexf"
...
print("\nOpen the HTML file in a browser to see the animated network!")
print("Use the slider or Play button to see Claude Code adoption spread over time.")

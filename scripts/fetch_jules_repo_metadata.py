#!/usr/bin/env python3
"""Parallel fetcher for Jules repo metadata using batched GraphQL alias queries.

Reads the unique repo list from jules_repos.csv and fetches
full metadata (33 columns + README) for each repo using parallel workers.

Usage:
    python scripts/fetch_jules_repo_metadata.py
    python scripts/fetch_jules_repo_metadata.py --no-readme   # skip README for speed
    python scripts/fetch_jules_repo_metadata.py --dry-run      # show plan only
"""
...
def load_jules_repos(repos_file: Path) -> List[str]:
    """Load unique repo names from Jules repos CSV file."""
    print(f"Loading Jules repos from {repos_file}...")
...
def merge_results(temp_dir: Path, output_file: Path) -> int:
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        for worker_file in sorted(temp_dir.glob("jules_repos_worker_*.jsonl")):
            count = 0
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch repo metadata for all Jules repos"
    )
    parser.add_argument(
        "--repos-file",
        type=str,
        default="data/output/jules_repos.csv",
        help="Input repos CSV file",
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
        default="data/output/jules_repos_metadata.jsonl",
    ...
    repo_list = load_jules_repos(repos_file)
    ...
    temp_dir = output_path.parent / "temp_jules_repo_workers"

    print("\n" + "=" * 70)
    print("JULES REPO METADATA FETCH PLAN")
    print("=" * 70)
    ...
    worker_args = []
    for i, chunk in enumerate(chunks):
        temp_file = temp_dir / f"jules_repos_worker_{i+1}.jsonl"
        worker_args.append((
            i + 1,
            tokens[i][1],
            chunk,
